...
public class PlaylistTrackAutocompleteHandler(IDbService dbService) : AutocompleteHandler
{
        ...
        IServiceProvider services)
    {
        ...
        var playlistName =
            autocompleteInteraction.Data.Options.First(opt => opt.Name == "playlist_name").Value as string ?? "";
        if (string.IsNullOrWhiteSpace(playlistName)) return AutocompletionResult.FromSuccess();

        var playlistExists = await dbService.PlaylistExistsAsync(context.Guild.Id, playlistName).ConfigureAwait(false);
        if (!playlistExists) return AutocompletionResult.FromSuccess();

        (await dbService.FindMatchingTracksForPlaylistAsync(context.Guild.Id, playlistName,
